...
export const metadata: Metadata = {
  title: "Pipeline Leak Detection Program Design: CPM vs Extended Sensor Approaches",
  description: "A field-oriented walkthrough of leak detection pipeline for pipeline integrity engineers, with decision matrices, code references, and the documentation au...",
  keywords: ["leak detection pipeline","CPM leak detection","fiber optic pipeline","API 1130"],
  alternates: { canonical: "https://pipeline-integrity-guide.vercel.app/case-studies/pipeline-leak-detection-program-design-cpm-vs-extended" },
  openGraph: {
    title: "Pipeline Leak Detection Program Design: CPM vs Extended Sensor Approaches",
    description: "A field-oriented walkthrough of leak detection pipeline for pipeline integrity engineers, with decision matrices, code references, and the documentation au...",
    type: 'article',
    url: "https://pipeline-integrity-guide.vercel.app/case-studies/pipeline-leak-detection-program-design-cpm-vs-extended",
    siteName: "Pipeline Integrity Guide",
    locale: 'en_US',
    publishedTime: "2026-02-18T00:00:00.000Z",
    modifiedTime: "2026-02-18T00:00:00.000Z",
...
const jsonLd = {
  ...
  "@type": "Article",
  "headline": "Pipeline Leak Detection Program Design: CPM vs Extended Sensor Approaches",
  "description": "A field-oriented walkthrough of leak detection pipeline for pipeline integrity engineers, with decision matrices, code references, and the documentation au...",
  "author": {
    "@type": "Person",
  ...
  "mainEntityOfPage": {
    "@type": "WebPage",
    "@id": "https://pipeline-integrity-guide.vercel.app/case-studies/pipeline-leak-detection-program-design-cpm-vs-extended"
  },
  "keywords": "leak detection pipeline, CPM leak detection, fiber optic pipeline, API 1130"
};
  ...
  return (
    <article className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 prose prose-slate prose-lg">
      ...
      <nav className="breadcrumb text-sm text-gray-500 mb-6">
        ...
        <span> / </span>
        <span>Pipeline Leak Detection Program Design: CPM vs Extended Sensor Approaches</span>
      </nav>
      <h1>Pipeline Leak Detection Program Design: CPM vs Extended Sensor Approaches</h1>
      <p className="text-sm text-gray-500 mb-8">By Linnea Söderberg, MSc, IWE &middot; Published 2026-02-18</p>
      <div dangerouslySetInnerHTML={{ __html: `<p>This article walks through the practical decisions pipeline integrity engineers, midstream operators, regulators need to make when working on pipeline integrity & inspection. The goal is not theory — it is the kind of working knowledge an experienced inspector or engineer would share over a coffee on a job site, with enough rigor to point back to code and standard references when those decisions get challenged in audit. Throughout the piece we focus on the parts of leak detection pipeline that turn into real schedule or budget pain when they go wrong.</p><h2>Why this comes up so often in the field</h2>
<p>Ask any pipeline integrity engineers how often leak detection pipeline drives schedule pain and you will hear the same answer: more often than the project plan ever assumes. Part of the reason is that this kind of inspection sits at the intersection of multiple owners — operations, integrity, fabrication, and QA — and the ownership boundaries are rarely clean. When a problem surfaces, it is usually too late to plan; the team is already in execution mode and looking for guidance.</p>
...
<ul>
<li><strong>The decision points</strong> that drive most leak detection pipeline programs disputes between QA, integrity, and operations.</li>
...
<p>Below is the working matrix our team uses on day one of any leak detection pipeline engagement. It is deliberately simple because complexity here costs more than it saves.</p>
...
<p>Auditors do not argue with conclusions; they argue with the evidence behind them. For any defensible leak detection pipeline report, the evidence packet needs to include at minimum:</p>
<ul>
...
<li><strong>Letting one inspector own the data interpretation alone.</strong> Interpretation of leak detection pipeline should always have a second reviewer for any indication that drives a fitness or repair decision.</li>
...
<p>If we had to summarize leak detection pipeline in one line it would be this: <strong>the technique matters less than the decision discipline around it.</strong> Teams that consistently choose the right technique are usually teams that have invested in writing down their decision rationale, qualifying their procedures with care, and keeping their inspectors current. Equipment and software change every few years; that discipline does not.</p>
<p>Practical teams often cross-reference <a href="https://atlantisndt.com/digital-twins/battery-storage" rel="noopener">the battery storage digital twin page</a> to align their practice with what is already published in the wider community. Practical teams often cross-reference <a href="https://atlantisndt.com/consulting/rbi-program-design" rel="noopener">the NDT consulting page for rbi program design</a> to align their practice with what is already published in the wider community.</p><h3>How leak detection pipeline fits into the bigger picture</h3>
<p>It is easy to study leak detection pipeline as an isolated subject — most courses do exactly that — but the engineering value only appears when you place leak detection pipeline alongside the other levers your program already uses. Practical teams often cross-reference <a href="https://www.imca-int.com/" rel="noopener nofollow">the IMCA technical resources for offshore work</a> to align their practice with what is already published in the wider community. For most pipeline integrity engineers the question is not "what is leak detection pipeline?" but "where does leak detection pipeline sit in our existing program, and what does it replace or complement?". That framing usually changes the procurement conversation, the training conversation, and the audit conversation in the same direction.</p>
<p>If your team is being asked to justify investment in leak detection pipeline, the easiest place to start is a one-page side-by-side: current state, gap, expected uplift, and the specific risk ranking that improves. The numbers do not have to be precise; they have to be defensible.</p>
<h3>Operator behaviours that actually move the needle on CPM leak detection</h3>
<p>Three behaviours we see at high-performing operators consistently show up in CPM leak detection programs that are working: written rationale for every off-procedure decision; a standing peer-review for indications that drive a fitness call; and a quarterly close-out review where the integrity engineer and the inspection lead sit down with the raw data, not just the summary. None of these behaviours require new hardware. All three improve outcomes more than another tool purchase usually does.</p>
<p>What we do not see at high-performing operators is over-reliance on a single inspector or a single contractor. Cross-checking — even informally — is the cheapest insurance you can buy on a CPM leak detection program.</p>
<h3>Documentation patterns worth borrowing</h3>
<p>The best fiber optic pipeline documentation packages we have seen share three patterns:</p>
<ul>
...
<h3>How CPM leak detection fits into the bigger picture</h3>
<p>It is easy to study CPM leak detection as an isolated subject — most courses do exactly that — but the engineering value only appears when you place CPM leak detection alongside the other levers your program already uses. Practical teams often cross-reference <a href="https://www.imca-int.com/" rel="noopener nofollow">the IMCA technical resources for offshore work</a> to align their practice with what is already published in the wider community. For most pipeline integrity engineers the question is not "what is CPM leak detection?" but "where does CPM leak detection sit in our existing program, and what does it replace or complement?". That framing usually changes the procurement conversation, the training conversation, and the audit conversation in the same direction.</p>
<p>If your team is being asked to justify investment in CPM leak detection, the easiest place to start is a one-page side-by-side: current state, gap, expected uplift, and the specific risk ranking that improves. The numbers do not have to be precise; they have to be defensible.</p>
<h3>Operator behaviours that actually move the needle on fiber optic pipeline</h3>
<p>Three behaviours we see at high-performing operators consistently show up in fiber optic pipeline programs that are working: written rationale for every off-procedure decision; a standing peer-review for indications that drive a fitness call; and a quarterly close-out review where the integrity engineer and the inspection lead sit down with the raw data, not just the summary. None of these behaviours require new hardware. All three improve outcomes more than another tool purchase usually does.</p>
<p>What we do not see at high-performing operators is over-reliance on a single inspector or a single contractor. Cross-checking — even informally — is the cheapest insurance you can buy on a fiber optic pipeline program.</p>
<h3>Documentation patterns worth borrowing</h3>
<p>The best API 1130 documentation packages we have seen share three patterns:</p>
<ul>
<li>A short narrative section at the front of every report explaining the inspection objective in plain English. Auditors, regulators, and new hires all read the narrative first.</li>
<li>A consistent indication-numbering scheme that survives across multiple inspection campaigns, so an indication found in 2024 can be tracked through 2026 and 2028 without renaming.</li>
<li>An attached "open items" list with target dates, ownership, and the trigger that closes each item. This is the single most useful artifact for a turnaround manager.</li>
