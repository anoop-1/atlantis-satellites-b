...
export const metadata: Metadata = {
  title: "ILI Data Validation Workflow: Building an Anomaly Truth Table",
  description: "ILI Data Validation Workflow explained for pipeline integrity engineers — covering ILI validation, common pitfalls, and the data your audit packet needs.",
  keywords: ["ILI validation","pipeline anomaly verification","ILI dig verification","ILI POD"],
  alternates: { canonical: "https://pipeline-integrity-guide.vercel.app/case-studies/ili-data-validation-workflow-anomaly-truth-table" },
  openGraph: {
    title: "ILI Data Validation Workflow: Building an Anomaly Truth Table",
    description: "ILI Data Validation Workflow explained for pipeline integrity engineers — covering ILI validation, common pitfalls, and the data your audit packet needs.",
    type: 'article',
    url: "https://pipeline-integrity-guide.vercel.app/case-studies/ili-data-validation-workflow-anomaly-truth-table",
...
const jsonLd = {
  ...
  "@type": "Article",
  "headline": "ILI Data Validation Workflow: Building an Anomaly Truth Table",
  "description": "ILI Data Validation Workflow explained for pipeline integrity engineers — covering ILI validation, common pitfalls, and the data your audit packet needs.",
  "author": {
    "@type": "Person",
  ...
  "mainEntityOfPage": {
    "@type": "WebPage",
    "@id": "https://pipeline-integrity-guide.vercel.app/case-studies/ili-data-validation-workflow-anomaly-truth-table"
  },
  "keywords": "ILI validation, pipeline anomaly verification, ILI dig verification, ILI POD"
};
  ...
  return (
    <article className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 prose prose-slate prose-lg">
      ...
      <nav className="breadcrumb text-sm text-gray-500 mb-6">
        ...
        <span> / </span>
        <span>ILI Data Validation Workflow: Building an Anomaly Truth Table</span>
      </nav>
      <h1>ILI Data Validation Workflow: Building an Anomaly Truth Table</h1>
      <p className="text-sm text-gray-500 mb-8">By Hector Alvarez, NACE-certified Pipeline Inspector &middot; Published 2025-05-15</p>
      <div dangerouslySetInnerHTML={{ __html: `<p>This article walks through the practical decisions pipeline integrity engineers, midstream operators, regulators need to make when working on pipeline integrity & inspection. The goal is not theory — it is the kind of working knowledge an experienced inspector or engineer would share over a coffee on a job site, with enough rigor to point back to code and standard references when those decisions get challenged in audit. Throughout the piece we focus on the parts of ili validation that turn into real schedule or budget pain when they go wrong.</p><h2>Why this comes up so often in the field</h2>
<p>Ask any pipeline integrity engineers how often ili validation drives schedule pain and you will hear the same answer: more often than the project plan ever assumes. Part of the reason is that this kind of inspection sits at the intersection of multiple owners — operations, integrity, fabrication, and QA — and the ownership boundaries are rarely clean. When a problem surfaces, it is usually too late to plan; the team is already in execution mode and looking for guidance.</p>
...
<ul>
<li><strong>The decision points</strong> that drive most ili validation programs disputes between QA, integrity, and operations.</li>
...
<p>Below is the working matrix our team uses on day one of any ili validation engagement. It is deliberately simple because complexity here costs more than it saves.</p>
...
<p>The textbook approach to ili validation usually points engineers at one or two techniques. The field reality is messier; the technique that ranks best on a comparison chart is often not the one that fits the access, the schedule, or the inspector skill mix actually available on the day. We walk through three patterns we see repeatedly.</p>
...
<p>Post-repair inspection is the most procedural of the three patterns and the most likely to be weakened by schedule pressure. The required NDT is usually defined by the original construction code (ASME VIII, ASME XI, AWS D1.1, and so on), which is fine — but the decision about <em>how</em> to apply it under field conditions is rarely covered. Practical teams often cross-reference <a href="https://atlantisndt.com/erp/accounting-ndt-inspection-companies-riyadh" rel="noopener">Atlantis NDT's accounting ndt inspection companies riyadh ERP write-up</a> to align their practice with what is already published in the wider community.</p><h2>The data the audit will ask for</h2>
<p>Auditors do not argue with conclusions; they argue with the evidence behind them. For any defensible ili validation report, the evidence packet needs to include at minimum:</p>
<ul>
...
<li><strong>Letting one inspector own the data interpretation alone.</strong> Interpretation of ili validation should always have a second reviewer for any indication that drives a fitness or repair decision.</li>
...
<p>If we had to summarize ili validation in one line it would be this: <strong>the technique matters less than the decision discipline around it.</strong> Teams that consistently choose the right technique are usually teams that have invested in writing down their decision rationale, qualifying their procedures with care, and keeping their inspectors current. Equipment and software change every few years; that discipline does not.</p>
<p>Practical teams often cross-reference <a href="https://atlantisndt.com/ndt-training-sugar-land" rel="noopener">the ndt training sugar land training programme</a> to align their practice with what is already published in the wider community.</p><h3>How ILI validation fits into the bigger picture</h3>
<p>It is easy to study ILI validation as an isolated subject — most courses do exactly that — but the engineering value only appears when you place ILI validation alongside the other levers your program already uses. Practical teams often cross-reference <a href="https://www.imca-int.com/" rel="noopener nofollow">the IMCA technical resources for offshore work</a> to align their practice with what is already published in the wider community. For most pipeline integrity engineers the question is not "what is ILI validation?" but "where does ILI validation sit in our existing program, and what does it replace or complement?". That framing usually changes the procurement conversation, the training conversation, and the audit conversation in the same direction.</p>
<p>If your team is being asked to justify investment in ILI validation, the easiest place to start is a one-page side-by-side: current state, gap, expected uplift, and the specific risk ranking that improves. The numbers do not have to be precise; they have to be defensible.</p>
<h3>Operator behaviours that actually move the needle on pipeline anomaly verification</h3>
<p>Three behaviours we see at high-performing operators consistently show up in pipeline anomaly verification programs that are working: written rationale for every off-procedure decision; a standing peer-review for indications that drive a fitness call; and a quarterly close-out review where the integrity engineer and the inspection lead sit down with the raw data, not just the summary. None of these behaviours require new hardware. All three improve outcomes more than another tool purchase usually does.</p>
<p>What we do not see at high-performing operators is over-reliance on a single inspector or a single contractor. Cross-checking — even informally — is the cheapest insurance you can buy on a pipeline anomaly verification program.</p>
<h3>Documentation patterns worth borrowing</h3>
<p>The best ILI dig verification documentation packages we have seen share three patterns:</p>
<ul>
...
<h3>How pipeline anomaly verification fits into the bigger picture</h3>
<p>It is easy to study pipeline anomaly verification as an isolated subject — most courses do exactly that — but the engineering value only appears when you place pipeline anomaly verification alongside the other levers your program already uses. Practical teams often cross-reference <a href="https://www.imca-int.com/" rel="noopener nofollow">the IMCA technical resources for offshore work</a> to align their practice with what is already published in the wider community. For most pipeline integrity engineers the question is not "what is pipeline anomaly verification?" but "where does pipeline anomaly verification sit in our existing program, and what does it replace or complement?". That framing usually changes the procurement conversation, the training conversation, and the audit conversation in the same direction.</p>
<p>If your team is being asked to justify investment in pipeline anomaly verification, the easiest place to start is a one-page side-by-side: current state, gap, expected uplift, and the specific risk ranking that improves. The numbers do not have to be precise; they have to be defensible.</p>
